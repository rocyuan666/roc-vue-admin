<template>
  <div>
    <!-- 导航 -->
    <mbx-nav one-nav="商品管理" two-nav="添加商品"></mbx-nav>
    <!-- 卡片视图 -->
    <el-card>
      <!-- 提示 -->
  ...
  methods: {
    ...
    tabClicked () {
      ...
      if (this.activeIndex === '1') {
        // 动态参数 请求
        getParamsData(this.getCateId, 'many').then(res => {
          if (res.data.meta.status !== 200) {
         ...
         * 访问静态参数（商品属性）面板
         */
        getParamsData(this.getCateId, 'only').then(res => {
          if (res.data.meta.status !== 200) {
            return this.$message.error('获取动态参数列表失败！')
          }
    ...
    handleRemove(file) {
      ...
      console.log(file)
      // 2.从pics数组中，找到图片对应的索引值 findIndex检测x（每一项）.pic === filePath，就是删除的那个
    ...
    addGoods () {
      this.$refs.addFormRef.validate(valid => {
        ...
        // 解决办法: 导入包：lodash 方法（深拷贝）：cloneDeep(boj)
